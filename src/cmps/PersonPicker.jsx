    ...
    return <div className="person-edit-modal">
        <ul className="task-persons clean-list">
            {taskPersons?.map(person => {
                return <li key={person.id} className="task-person">
                    <img className="person-img" src={person.imgUrl} />
                    <h4 className="person-name">{person.fullName}</h4>
                    <button onClick={() => onRemovePerson(person.id)} className="delete-person-btn"><Xbutton /></button>
                </li>
            })}

        </ul>
        <form className="search-person-form">
            <input className="search-person-input"
                type="text"
                placeholder="Search names"
            >
            </input>
            <button className="search-btn"><SmallSearchIcon /></button>
        </form>
        ...
        <ul className="suggested-people-list clean-list">
            {suggestedPersons?.map(person => {
                return <li key={person.id} onClick={() => onAddPerson(person.id)} className="suggested-person">
