...
export function EditLabels({ options, from }) {
    ...
    async function onUpdateLabelTitle(txt) {
        ...
        const boardToSave = { ...board, [from]: [...board[from].map(s => s.id === labelToEdit.id ? updatedLabel : s)] }
        try {
    ...
    return <div className="edit-labels-container">
        ...
        {options.map(item => <div key={item.id}
            ...
            onClick={() => { if (!labelToEdit || labelToEdit.id !== item.id) setLabelToEdit(item) }}
        >
            <div className="label flex">
                ...
                <div onClick={() => { setIsColorEdit(false) }}>
                    <EditableText
                        prevTxt={item.title}
            ...
            <button onClick={() => onRemoveLabel(item.id)}
        ...
        <button onClick={onAddNewLabel}
        className="new-label-btn">+ New label</button>
    </div>
}